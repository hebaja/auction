...
export default {
  ...
  created() {
    // setInterval(() => {
    //   if(this.groupActiveArray.length) {
    //     this.mutableGroupPlayers.forEach(group => {
    //       if(group.active) {
    //         console.log(this.mutableGroupPlayers.indexOf(group))
    //       }
    //     })
    //   }
    // },5000)
  ...
  computed: {
    ...
    playersLink: function () {
      var playersLink = [];
      if(this.mutableGroupPlayers) {
        this.mutableGroupPlayers.forEach((group) => {
          group.players.forEach((player) => {
            playersLink.push({
              id: player.id,
              playerName: player.playerName,
              link: "https://joy-auction.herokuapp.com/?playerId=" + player.id,
            })
          })
        })
      }
  ...
  methods: {
    ...
    deleteGroup: function (auctioneerId, groupPlayerId) {
